...
const seeAll = async (req, res) => {
  try {
    const { page, limit, status, type, priority, sort } = req.query;

    const pageNumber = Number(page) || 1;

    const limitNumber = Number(limit) || 10;

    const informations = await seeAllService({
      pageNumber,
      limitNumber,
      status,
      type,
...
const statsTickets = async (req, res) => {
  try {
    ...
    const informations = await statsTicketService(id);

    res.json(informations);
  } catch (error) {
    console.log(error);

    res.status(500).json("erreur stats tickets");
  }
};

const adminStats = async (req, res) => {
  try {
    const informations = await adminStatsService();

    res.json(informations);
  } catch (error) {
    console.log(error);

    res.status(500).json("erreur admin stats");
  }
};
