...
function ConnaissancesEdit() {
  const { editId } = useParams();
  const navigate = useNavigate();

  const { connaissance, voirUneConnaissance, ModifierConnaissance } =
  ...
  useEffect(() => {
    voirUneConnaissance(editId);
  }, [editId]);

  useEffect(() => {
    if (connaissance) {
      setTitle(connaissance.title || "");
      setContent(connaissance.content || "");
      setCategoryId(connaissance.categoryId || "");
    }
  }, [connaissance]);
  ...
  const handleSubmit = async (e) => {
    ...
    await ModifierConnaissance(Number(editId), title, content, categoryId);
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <div className="bg-slate-50 border rounded-3xl p-6">
            ...
            <p className="text-gray-500 mt-2">
              {categoryList.find((cat) => cat.id === Number(categoryId))?.name}
            </p>
...
export default ConnaissancesEdit;
